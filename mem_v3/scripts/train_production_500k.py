...
def main():
    parser = argparse.ArgumentParser(description="MEM Orchestrator - Production 500k Pre-Training Run")
    parser.add_argument("--steps", type=int, default=500000, help="Total training steps (default: 500,000)")
    parser.add_argument("--seq-len", type=int, default=512, help="Context sequence length (default: 512)")
    ...
    parser.add_argument("--dataset", default="HuggingFaceFW/fineweb", help="Primary dataset (default: HuggingFaceFW/fineweb raw web)")
    parser.add_argument("--dataset-config", default="sample-10BT", help="Dataset configuration (default: sample-10BT)")
    parser.add_argument("--fallback-dataset", default="roneneldan/TinyStories", help="Fallback dataset if network drops")
    parser.add_argument("--start-lane", default="aggressive_seq512_zero0_gacc4", help="Initial operating lane (default: aggressive_seq512_zero0_gacc4)")
    parser.add_argument("--checkpoint-interval", type=int, default=1000, help="Steps between atomic checkpoints (default: 1000)")
    parser.add_argument("--checkpoint-label", default="v90", help="Checkpoint series label (default: v90)")
    parser.add_argument("--val-interval", type=int, default=1000, help="Steps between validation evaluations (default: 1000)")
    ...
    parser.add_argument("--clean", action="store_true", help="Start completely fresh from step 0 (archives old v90 checkpoints)")
    ...
    if args.clean and ckpt_root.exists():
        archive_dir = ckpt_root / f"archive_v90_{time.strftime('%Y%m%d_%H%M%S')}"
        archive_dir.mkdir(parents=True, exist_ok=True)
        import shutil
        for item in ckpt_root.iterdir():
            if item.is_dir() and item.name.startswith("v90_live_"):
                shutil.move(str(item), str(archive_dir / item.name))
            elif item.is_file() and item.name.startswith("v90_"):
                shutil.move(str(item), str(archive_dir / item.name))
        print(f">>> [CLEAN START] Checkpoints v90 arquivados com sucesso em: {archive_dir.name}\n")
    ...
    if args.resume_latest and not resume_target and not args.clean:
        v90_ptr = ckpt_root / f"{args.checkpoint_label}_latest.txt"
        if v90_ptr.exists():
            ptr_content = v90_ptr.read_text(encoding="utf-8").strip()
            if ptr_content and Path(ptr_content).exists():
                resume_target = ptr_content
        if not resume_target:
            resume_target = "latest"

    print("\n" + "="*84, flush=True)
    print(f"{BOLD}{CYAN}  MEM ORCHESTRATOR - PRODUCTION PRE-TRAINING RUN (500,000 STEPS){RESET}", flush=True)
    ...
    print(f"  Context Window:      {args.seq_len} Tokens (Extended Long-Range Rationale)", flush=True)
    print(f"  Dataset Streaming:   {args.dataset} [{args.dataset_config}] - Real Web Raw Data", flush=True)
    print(f"  Target Global Steps: {args.steps:,} steps", flush=True)
    print(f"  Checkpoint Cadence:  Every {args.checkpoint_interval:,} steps (Series: {args.checkpoint_label})", flush=True)
    ...
    print(f"  Zero-OOM Governance: Active (Lane: {args.start_lane})", flush=True)
    print("="*84 + "\n", flush=True)
    ...
    evidence_dir = Path(_root) / "evidence" / f"train_500k_{ts}"
    ...
    try:
        ...
        print(f"{CYAN}  .\\.venv\\Scripts\\python.exe scripts\\train_production_500k.py --resume-latest{RESET}", flush=True)
        print("!"*84 + "\n", flush=True)
        return

    print("\n" + "="*84, flush=True)
    print(f"{BOLD}{GREEN}  TREINO DE 500.000 STEPS CONCLUÍDO COM SUCESSO!{RESET}", flush=True)
